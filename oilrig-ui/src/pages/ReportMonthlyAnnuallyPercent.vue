<template>
    <div class="p-6">
      <h1 class="text-2xl font-bold mb-4">Monthly Oil % vs Annual Planned</h1>
      ...
      <div class="mb-4 flex items-center gap-4">
        ...
        <button
          @click="fetchMonthlyAnnualPercent"
      ...
      <div v-else-if="reports.length === 0" class="text-red-500">No data found for selected month/year.</div>
  ...
  const fetchMonthlyAnnualPercent = async () => {
    ...
    try {
      const res = await axios.get('/api/reports/percent/monthly-vs-annual', {
        params: { year: year.value, month: month.value }
      })
      reports.value = res.data
    } catch (err) {
      console.error('Error fetching monthly vs annual % report:', err)
    } finally {
      loading.value = false
    }
  }
